  ...
  if (req.params.id) {
    ...
    Appointment.findById(id)
      ...
      .catch((err) => {
        res
          .status(500)
          .send({ message: "Erro retrieving appointment with id " + id });
      });
  } else {
    ...
    Promise.all([
      ...
      Appointment.find()
        ...
        .skip((page - 1) * limit)
        .then((appointment) => {
          data.current = appointment.length;
...
exports.update = (req, res) => {
  ...
  Appointment.findByIdAndUpdate(id, req.body, {
    ...
    .then((data) => {
      if (!data) {
        res.status(404).send({
          message: `Cannot Update appointment with ${id}. Maybe appointment not found!`,
        });
      } else {
        res.send(data);
      }
